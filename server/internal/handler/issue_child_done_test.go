...
func newChildDoneFixture(t *testing.T, parentStatus string) childDoneFixture {
	...
	req = newRequest("POST", "/api/issues?workspace_id="+testWorkspaceID, map[string]any{
		...
		// I4127.DP: in_progress requires a valid assignee. The child is the
		// one whose transitions drive these tests, so it gets the seeded
		// test agent; the parent's assignee (or lack of one) is the variable
