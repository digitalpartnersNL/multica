...
Requires the directory to be a git repository with at least one commit. If it is not, tasks fail with an explicit error rather than quietly falling back to serial execution. A runtime too old to implement the mode also fails the task rather than running in place — asking for `worktree` is asking for isolation, so silently editing your working copy instead is never the fallback.
